...
<template>
    <Layout>
        ...
        <div v-if="props.resume.summaries?.length" class="section summary">
            ...
            <p v-for="(summary, index) in props.resume.summaries" :key="index">
                {{ summary.body }}
            </p>
        </div>
        ...
        <div v-if="props.resume.workExperiences?.length" class="section">
            ...
            <div
                v-for="(work, index) in props.resume.workExperiences"
                :key="index"
            ...
            >
                ...
                <div class="meta">
                    ...
                    {{
                        new Date(work.start).toLocaleString('default', {
                            month: 'short',
                            year: 'numeric',
                        })
                    }}
                    ...
                    {{
                        work.end
                            ? new Date(work.end).toLocaleString('default', {
                                  month: 'short',
                                  year: 'numeric',
                              })
                            : 'Present'
        ...
        <div v-if="props.resume.skills?.length" class="section">
            ...
            <div class="skills-list">
                ...
                >
                    {{ skill.name }}
                    <span v-if="skill.proficiency"
                        >({{ skill.proficiency.value }})</span
                    >
                </div>
            </div>
        </div>
        ...
        <div v-if="props.resume.projects?.length" class="section">
            ...
            >
                <h3>{{ project.title }}</h3>
                <div v-if="project.role" class="meta">
                    Role: {{ project.role }}
                </div>
        ...
        <div v-if="props.resume.certificates?.length" class="section">
            ...
            <ul class="certificates-list">
                <li
                    v-for="(cert, index) in props.resume.certificates"
                    :key="index"
                >
                    <strong>{{ cert.name }}</strong>
                    <span v-if="cert.description"
        ...
        <div v-if="props.resume.references?.length" class="section">
            ...
            <ul class="references-list">
                <li
                    v-for="(ref, index) in props.resume.references"
                    :key="index"
                >
        ...
        <div v-if="props.resume.tags?.length" class="tags">
            <span v-for="(tag, index) in props.resume.tags" :key="index">{{
                tag
            }}</span>
        </div>
    </Layout>
